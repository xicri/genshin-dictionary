...
describe("The Genshin English Dictionary", () => {
  ...
  test("search by English", async ({ page }) => {
    ...
    if (!searchBox) {
      throw new AssertionError({ message: "input[name='searchbox'] was not found." });
    }
  ...
  test("pinyin is properly displayed on Chinese", async ({ page }) => {
    ...
    expect(await chinese?.innerHTML()).toBe("珠<ruby>钿<rp>(</rp><rt style=\"font-weight: lighter;\">diàn</rt><rp>)</rp></ruby><ruby>舫<rp>(</rp><rt style=\"font-weight: lighter;\">fǎng</rt><rp>)</rp></ruby>");
